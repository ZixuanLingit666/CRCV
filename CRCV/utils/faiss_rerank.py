...
def compute_jaccard_distance(target_features, cam_labels, num_cams, use_cam, k1=20, k2=6, print_flag=True, search_option=0, use_float16=False):
    end = time.time()
    if print_flag:
    ...
    # todo S^cam
    if use_cam:
        cam_dist = torch.zeros([num_cams, num_cams])

        for cam1 in range(num_cams):
            for cam2 in range(cam1 + 1):
                cam1_mask = cam_labels.eq(cam1)     # todo camera 1 下的样本
                cam2_mask = cam_labels.eq(cam2)     # todo camera 2 下的样本
                
                # todo
                offset = torch.mean(target_features[cam1_mask].mm(target_features[cam2_mask].t()))
                cam_dist[cam1][cam2] = offset
                cam_dist[cam2][cam1] = offset

        # todo 新的相似度矩阵
        cam_offset = torch.zeros(num_cams, N).cuda()        # N_cam x N
        for i in range(num_cams):
            cam_offset[i] = 1 - torch.sigmoid(cam_dist[i][cam_labels])
        del cam_dist
        
        initial_rank = np.zeros(shape=(N, N))
        for i in range(N):
            initial_rank[i] = np.argpartition(2 - 2 * (target_features[i].unsqueeze(0).mm(target_features.t()) * cam_offset[cam_labels[i]].unsqueeze(0)
         ).cpu(), range(1, k1 + 1))
    ...
    for i in range(N):
        ...
        # todo
        if use_cam:
            dist = 2-2 * cam_offset[cam_labels[i]][k_reciprocal_expansion_index].unsqueeze(0) * torch.mm(target_features[i].unsqueeze(0).contiguous(), target_features[k_reciprocal_expansion_index].t())
        else:
